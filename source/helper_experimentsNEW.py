...
def find_best_hyper_params_bayesian_search(dataset_name, n_iter=100, search_max_epoch=100, cv_count=3, features_to_use_str="min,max,mean"):
    train, y_train, test, y_test = load_dataset(dataset_name)
    y_train = torch.from_numpy(y_train).float()
    y_train_labels = np.argmax(y_train, axis=1)
    n_classes = y_train.shape[1]

    net, scheduler = get_skorch_regularized_classifier(ShapeletNN, search_max_epoch)
    ...
    for epoch in range(search_max_epoch):
        net.fit(train, y_train_labels)
...
def get_skorch_regularized_classifier(nn_shapelet_generator, max_epochs):
    ...
    # Get the optimizer from the net object
    optimizer = net.optimizer

    # Check if the optimizer is an instance of torch.optim.Optimizer
    if not isinstance(optimizer, torch.optim.Optimizer):
        raise TypeError("The optimizer is not an instance of torch.optim.Optimizer")
