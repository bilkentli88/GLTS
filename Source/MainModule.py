...
def find_best_hyper_params_optuna_search_default(dataset_name
                                                 , n_iter=100
                                                 , search_max_epoch=100
                                                 , cv_count=3
                                                 , features_to_use_str="min,max,mean"
    ...
    return find_best_hyper_params_optuna_search(dataset_name=dataset_name
                                                , sampler=None
                                                , n_iter=n_iter
                                                , search_max_epoch=search_max_epoch
                                                , cv_count=cv_count
...
def find_best_hyper_params_optuna_search_NSGAIISampler(dataset_name, n_iter=100
                                                       , search_max_epoch=100
                                                       , cv_count=3
                                                       , features_to_use_str="min,max,mean"
    ...
    return find_best_hyper_params_optuna_search(dataset_name=dataset_name
                                                , sampler="NSGAIISampler", n_iter=n_iter
                                                , search_max_epoch=search_max_epoch
                                                , cv_count=cv_count
                                                , features_to_use_str=features_to_use_str
...
def find_best_hyper_params_optuna_search_CmaEsSampler(dataset_name, n_iter=100
                                                      , search_max_epoch=100
                                                      , cv_count=3
                                                      , features_to_use_str="min,max,mean"
    ...
    return find_best_hyper_params_optuna_search(dataset_name=dataset_name
                                                , sampler="CmaEsSampler", n_iter=n_iter
                                                , search_max_epoch=search_max_epoch
                                                , cv_count=cv_count
                                                , features_to_use_str=features_to_use_str
...
def find_best_hyper_params_optuna_search_MOTPESampler(dataset_name, n_iter=100
                                                      , search_max_epoch=100
                                                      , cv_count=3
                                                      , features_to_use_str="min,max,mean"
...
def find_best_hyper_params_optuna_search_PartialFixedSampler(dataset_name, n_iter=100
                                                             , search_max_epoch=100
                                                             , cv_count=3
                                                             , features_to_use_str="min,max,mean"
...
def find_best_hyper_params_optuna_search(dataset_name
                                         , sampler=None
                                         , n_iter=100
                                         , search_max_epoch=100
                                         , cv_count=3
    ...
    # print("list_bag_sizes",list_bag_sizes)
    liste_cv_count = [cv_count]
    liste_dataset_name = [dataset_name]
    liste_search_max_epoch = [search_max_epoch]
    liste_features_to_use_str = [features_to_use_str]

    def optuna_objective(trial):
        stride_ratio = trial.suggest_categorical("stride_ratio", LIST_STRIDE_RATIOS)
        n_prototypes = trial.suggest_categorical("n_prototypes", LIST_N_PROTOTYPES)
        cv_count = trial.suggest_categorical("cv_count", liste_cv_count)
        dataset_name = trial.suggest_categorical("dataset_name", liste_dataset_name)
        max_epoch = trial.suggest_categorical("max_epoch", liste_search_max_epoch)
        features_to_use_str = trial.suggest_categorical(
            "features_to_use_str", liste_features_to_use_str)
        bag_size = trial.suggest_categorical("bag_size", list_bag_sizes)
        lr = trial.suggest_categorical("lr", LIST_LEARNING_RATES)
        train, y_train, test, y_test = load_dataset(dataset_name)
        y_train = torch.from_numpy(y_train).float()
        y_train_labels = np.argmax(y_train, axis=1)
        n_classes = y_train.shape[1]
        ...
        accuracy = cross_val_score(net, train, y_train_labels, cv=cv_count).mean()
    ...
    elif sampler == "MOTPESampler":
        study = optuna.create_study(direction="maximize", sampler=optuna.samplers.MOTPESampler())
    elif sampler == "CmaEsSampler":
        study = optuna.create_study(direction="maximize", sampler=optuna.samplers.CmaEsSampler())
    elif sampler == "PartialFixedSampler":
        study = optuna.create_study(direction="maximize", sampler=optuna.samplers.PartialFixedSampler())
    elif sampler == "NSGAIISampler":
        study = optuna.create_study(direction="maximize", sampler=optuna.samplers.NSGAIISampler())
    else:
        ValueError("Samples is not recognized", sampler)
    ...
    d = {}
    d["dataset_name"] = dataset_name
    d["search_type"] = "OptunaSearch"
    d["features_to_use"] = features_to_use_str
    d["best_hyper_params"] = str(best)
    d["bag_size"] = best["bag_size"]
    d["features_to_use"] = best["features_to_use_str"]
    d["n_classes"] = n_classes
    d["n_prototypes"] = best["n_prototypes"]
    d["stride_ratio"] = best["stride_ratio"]
    d["best_score"] = study.best_value
    d["n_iter"] = n_iter
    d["cv_count"] = cv_count
...
dispatcher = { 'BayesSearch' : find_best_hyper_params_bayesian_search
			...
			,"HyperoptSearchPartial":find_best_hyper_params_hyperopt_search_partial
            		,"OptunaSearchDefault": find_best_hyper_params_optuna_search_default
            		,"OptunaSearchNSGAIISampler": find_best_hyper_params_optuna_search_NSGAIISampler
           		 ,"OptunaSearchCmaEsSampler": find_best_hyper_params_optuna_search_CmaEsSampler
            		,"OptunaSearchMOTPESampler": find_best_hyper_params_optuna_search_MOTPESampler
			,"OptunaSearchPartialFixedSampler": find_best_hyper_params_optuna_search_PartialFixedSampler
